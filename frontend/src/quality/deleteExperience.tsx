...
interface DeleteExperienceInterface {
...
export function DeleteExperience(props: DeleteExperienceInterface) {
  const { experience, experiences, setExperiences, messageToShowUser } = props;
  ...
  const handleLookupCallback = (response: any, status: number) => {
    if (status === 204) {
      ...
      messageToShowUser(
        t("objectRemoved", { title: experience.title }),
        "alert alert-success"
      );
    }
  };
  const handleOnClick = (
    ...
    const confirmation = window.confirm(
      t("removeConfirm", { title: experience.title })
    );
    if (confirmation) {
      const endpoint = `${paths_dict.experience}${experience.id}/`;
      lookup(
        "DELETE",
        endpoint,
